...
void internal_timer::process_event_message(events::event_message* message) {
	if (!message) {
		return;
	}
...
void internal_timer::process_init_event(events::event_message* message) {
...
void internal_timer::process_start_event(events::event_message* message) {
	uint64_t instance_id = *static_cast<uint64_t*>(message->event_data[0]);
...
void internal_timer::process_stop_event(events::event_message* message) {
	uint64_t instance_id = *static_cast<uint64_t*>(message->event_data[0]);
...
void internal_timer::process_discard_event(events::event_message* message) {
	uint64_t instance_id = *static_cast<uint64_t*>(message->event_data[0]);
	instances.erase(instance_id);
}

void internal_timer::process_heartbeat_event(events::event_message* message) {
	uint64_t instance_id = *static_cast<uint64_t*>(message->event_data[0]);
