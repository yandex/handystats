...
static void check_full_json_dump(const std::string& string_dump) {
	...
	for (rapidjson::Value::MemberIterator iter = dump.MemberBegin(); iter != dump.MemberEnd(); ++iter) {
		...
		ASSERT_TRUE(iter->value.FindMember("type") != iter->value.MemberEnd());
		...
		if (type == "gauge") {
//			ASSERT_TRUE(iter->value.FindMember("timestamp") != iter->value.MemberEnd());
//			ASSERT_TRUE(iter->value.FindMember("value") != iter->value.MemberEnd());

//			ASSERT_TRUE(iter->value.FindMember("values") != iter->value.MemberEnd());
		}
