...
TEST(CounterEventsTest, TestCounterInitEvent) {
	...
	ASSERT_EQ(*static_cast<handystats::metrics::counter::value_type*>(message->event_data), init_value);
}
...
TEST(CounterEventsTest, TestCounterIncrementEvent) {
	...
	ASSERT_EQ(*static_cast<handystats::metrics::counter::value_type*>(message->event_data), value);
}
