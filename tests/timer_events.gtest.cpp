...
TEST_F(TimerEventsTest, TestTimerInitEventWithDefaultInstance) {
	...
	ASSERT_STREQ(message->destination_name, timer_name);
...
TEST_F(TimerEventsTest, TestTimerSetEvent) {
	...
	ASSERT_EQ(reinterpret_cast<int64_t>(message->event_data), duration.count(handystats::metrics::timer::value_unit));
